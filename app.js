...
const DB={
  ...
  migrate(d){
    ...
    d.products=(d.products||[]).map(p=>({stock:0,costo:0,descr:'',categoria:'General',...p}));
...
const Inventario={
  ...
  guardar(){
    ...
    const p={sku,nombre,categoria:(document.getElementById('prodCategoria').value||'').trim()||'General',precio:parseFloat(document.getElementById('prodPrecio').value||'0'),costo:parseFloat(document.getElementById('prodCosto').value||'0'),stock:parseInt(document.getElementById('prodStock').value||'0',10),descr:(document.getElementById('prodDescr').value||'').trim()};
